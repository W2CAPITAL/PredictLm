  ...
  async function run(){
    ...
    try{
      ...
      if(s.mode==='review'&&s.provider==='predict-core'){
        ...
        result={explanation:'DeepThink classificou o pedido como “'+info.intent+'” ('+Math.round(info.confidence*100)+'%). Plano local criado sem executar mudanças.',plan:['Interpretar objetivo','Extrair funcionalidades: '+info.features.join(', '),'Selecionar blueprint funcional','Gerar estado e ações reais','Validar preview','Executar Council antes do ship'],files:[]};
      } else if(s.provider==='predict-core'){
        result=runPredictCore(task,files,s.deepThinkLevel);
      } else {
        try{
          ...
          result=runPredictCore(task,files,s.deepThinkLevel);
          result.explanation='O provider opcional falhou ('+(externalError?.message||'indisponível')+'). Fallback automático para Predict DeepThink: '+result.explanation;
