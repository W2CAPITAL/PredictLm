...
const labels:Record<AnimalBackend,string>={auto:'IA visual · automático',browser:'Somente navegador · classificador leve','hog-svm':'HOG + SVM · KamgangAnthony','pytorch-resnet':'ResNet · rt75272','keras-resnet':'Gato, cachorro e cobra · AlvaroVasquezAI'};
export function AnimalVisionPanel({onChat}:{onChat:(text:string)=>void}){
  ...
  const [backend,setBackend]=useState<AnimalBackend>('auto');
  ...
  return <section className="animal-vision">
    ...
    <div className="animal-vision-grid">
      <div className="animal-vision-card">
        ...
        <p className="animal-note">{backend==='auto'?'O PredictLM tenta compreensão visual multimodal primeiro e usa o classificador local apenas se ela não estiver disponível.':backend==='browser'?'A foto fica neste navegador. Este modo é privado, mas o classificador leve reconhece categorias limitadas e pode errar espécies.':'A foto será enviada ao serviço de visão configurado, somente para esta análise.'}</p>
      ...
      <div className="animal-vision-card animal-result" aria-live="polite">
        {result?<><span className="animal-eyebrow">Resultado da análise</span><h2>{result.verdict==='likely-animal'?result.predictions[0].label:result.verdict==='not-animal'?'Categoria não animal':'Identificação inconclusiva'}</h2>{result.scientificName?<p className="animal-note"><b>Nome científico provável:</b> {result.scientificName}</p>:null}{result.description?<p>{result.description}</p>:null}<ol>{result.predictions.map((p,i)=><li key={i}><div><span>{p.label}</span><b>{(p.score*100).toFixed(1)}%</b></div><progress max={1} value={p.score}/></li>)}</ol><p className="animal-note">Escores do modelo, não garantia de identificação. Fotos com vários animais, desenhos ou espécies desconhecidas podem gerar erros.</p><button className="animal-action" onClick={()=>onChat(animalSummary(result))}><Send size={15}/>Levar resultado ao Chat</button></>:<><Eye size={42}/><h2>Uma foto, várias hipóteses</h2><p>O resultado aparece aqui após a análise. Use uma foto nítida com o animal em destaque.</p><p className="animal-note">Os modelos dos três projetos ficam disponíveis quando seus pesos estão configurados. O modo do navegador funciona de forma independente.</p></>}
      </div>
    </div>
  </section>;
}
