...
export function canonicalMatchupLock(input:string){
  ...
  return [
    ...
    'LEFT THIRD: Naruto Uzumaki is visibly associated with a gigantic complete golden-orange Kurama chakra fox avatar. Kurama must read as a fox/beast with a clear draconic-NOT-allowed fox head, powerful torso, canonical black chakra markings and NINE distinct flowing tails. Keep Naruto recognizable and separate from Kurama rather than turning Kurama into a humanoid Naruto clone.',
    'RIGHT THIRD: Sasuke Uchiha is visibly associated with a gigantic complete violet Perfect Susanoo. Susanoo must be a full armored winged humanoid chakra avatar with recognizable helmet/face structure, broad armor plates, BOTH wings visible when framing permits and a luminous sword. Never reduce it to purple smoke, a generic demon, mecha or cropped torso.',
