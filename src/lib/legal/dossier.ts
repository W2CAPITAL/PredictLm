...
import { assessFraudRisk, type FraudTransaction } from '@/lib/security/fraud-defense';
...
export interface LegalDossierEvidence{
  ...
  fraudArtifacts?:{kind:'message'|'url'|'payment'|'identity'|'other';value:string;source?:string}[];
...
export function createLegalDossier(bundle:LegalProcessBundle,options?:LegalDossierOptions){
  ...
  const fraudTexts=[
    ...(evidence?.documents||[]).map(x=>x.summary),
    ...(evidence?.quotes||[]).map(x=>x.text),
    ...(evidence?.fraudArtifacts||[]).filter(x=>x.kind!=='url').map(x=>x.value)
  ].filter(Boolean);
  const fraudUrls=[
    ...(evidence?.fraudArtifacts||[]).filter(x=>x.kind==='url').map(x=>x.value),
    ...fraudTexts.flatMap(x=>String(x).match(/https?:\/\/[^\s<>"')]+/g)||[])
  ];
  const fraudAssessment=assessFraudRisk({texts:fraudTexts,urls:fraudUrls,transactions:evidence?.fraudTransactions||[]});
  ...
  const fraudSignals=fraudAssessment.signals.map(x=>'<li><b>'+esc(x.label)+'</b> — '+esc(x.evidence)+'</li>').join('');
  const fraudActions=fraudAssessment.recommendations.map(x=>'<li>'+esc(x)+'</li>').join('');
  const fraudHtml='<div class="fraud-card"><div class="fraud-score"><span>Triagem</span><b>'+fraudAssessment.score+'/100 · '+esc(fraudAssessment.level.toUpperCase())+'</b></div><p>'+esc(fraudAssessment.disclaimer)+'</p>'+(fraudSignals?'<h3>Sinais encontrados</h3><ul>'+fraudSignals+'</ul>':'<p>Nenhum sinal heurístico relevante foi detectado nos artefatos fornecidos. Isso não comprova autenticidade.</p>')+'<h3>Verificações recomendadas</h3><ul>'+fraudActions+'</ul></div>';
  ...
  '<nav class="nav-bar"><div class="nav-inner"><button class="nav-btn active" data-target="visao">Visão geral</button><button class="nav-btn" data-target="evidencias">Evidências</button><button class="nav-btn" data-target="timeline">Linha do tempo</button><button class="nav-btn" data-target="forcas">A favor / contra</button><button class="nav-btn" data-target="falhas">Pontos críticos</button><button class="nav-btn" data-target="riscos">Riscos</button><button class="nav-btn" data-target="fraude">Fraude</button><button class="nav-btn" data-target="council">Council</button><button class="nav-btn" data-target="acoes">Ações</button><button class="nav-btn" data-target="fontes">Fontes</button></div></nav>'+
  '<main class="main">'+
  ...
  '<section id="fraude"><span class="eyebrow">07 · FRAUDE / AUTENTICIDADE</span><h2>Triagem defensiva de golpe, fraude e abuso</h2><p class="lead">Cruza sinais de engenharia social, credenciais, pagamento, links e padrões de rede. Nenhum sinal isolado é tratado como prova de crime.</p>'+fraudHtml+'</section>'+
  '<section id="council"><span class="eyebrow">08 · COUNCIL</span><h2>Análise multi-lente</h2><div class="lenses">'+(lenses||'<article class="lens info"><div class="lens-h">Sem lentes</div><div class="lens-b">Não houve sinal suficiente para gerar lentes específicas.</div></article>')+'</div><div class="synth"><h3>Síntese do Chair</h3><p><strong>Consenso:</strong> '+esc(chair.consensus||'—')+'</p><p><strong>Divergência:</strong> '+esc(chair.divergence||'—')+'</p><p><strong>Recomendação:</strong> '+esc(chair.recommendation||'—')+'</p><p><strong>Risco residual:</strong> '+esc(chair.residualRisk||'—')+'</p></div></section>'+
  '<section id="acoes"><span class="eyebrow">09 · PLANO DE AÇÃO</span><h2>Próximos passos</h2><div class="rec-group"><span class="phase now">Imediato</span>'+rec('immediate')+'</div><div class="rec-group"><span class="phase mid">Médio prazo</span>'+rec('medium')+'</div><div class="rec-group"><span class="phase no">Não fazer</span>'+rec('avoid')+'</div></section>'+
  attack+
  '<section id="fontes"><span class="eyebrow">'+(aggressive?'11':'10')+' · FONTES E LIMITAÇÕES</span><h2>Confiança por fonte</h2><table class="source-table"><thead><tr><th>Fonte</th><th>Confiança</th><th>Detalhe</th></tr></thead><tbody>'+confidence+'</tbody></table><h3>Publicações DJEN</h3>'+(pubs||'<p>Sem publicações DJEN nesta consulta.</p>')+'<h3>Caveats</h3><ul>'+caveats+'</ul><p class="foot">Gerado em '+esc(date(bundle.fetchedAt))+'. Dados públicos não substituem autos completos nem parecer jurídico formal. O gerador não inventa conteúdo para preencher lacunas documentais.</p></section>'+
