...
function crmApp(title:string,prompt:string=''){
  ...
  return [
    ...
    " const [integrations,setIntegrations]=useState({rest:true,supabase:false,stripe:false,webhook:false});",
    ...
    " useEffect(()=>{let live=true;setLoading(true);fetch(API+'/api/leads').then(r=>r.ok?r.json():Promise.reject(new Error('offline'))).then(rows=>{if(live&&Array.isArray(rows)&&rows.length){setLeads(rows.map(x=>({...x,value:Number(x.value||0),mrr:Number(x.mrr||0)})));setBackend('connected')}}).catch(()=>live&&setBackend('local')).finally(()=>live&&setLoading(false));return()=>{live=false}},[]);",
    ...
    " const exportCsv=()=>{const rows=[['Nome','Empresa','Email','Telefone','Status','Valor','MRR','Responsável'],...leads.map(x=>[x.name,x.company,x.email,x.phone,x.status,x.value,x.mrr,x.owner])];const csv=rows.map(r=>r.map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(';')).join('\\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download='crm-clientes.csv';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)};",
    ...
    "   {view==='Integrações'&&<div className='crm-content'><section className='integration-grid'>{[{id:'rest',name:'API REST',desc:'Sincroniza clientes com o backend exportado.'},{id:'supabase',name:'Supabase',desc:'Postgres/Auth opcional via variáveis de ambiente.'},{id:'stripe',name:'Stripe',desc:'Cobrança e pagamentos pelo backend.'},{id:'webhook',name:'Webhooks',desc:'Eventos de criação, atualização e fechamento.'}].map(x=><article className='crm-card' key={x.id}><div className='integration-icon'>{x.name[0]}</div><div><b>{x.name}</b><span>{x.desc}</span></div><button className={integrations[x.id]?'connected-btn':''} onClick={()=>setIntegrations(v=>({...v,[x.id]:!v[x.id]}))}>{integrations[x.id]?'Ativa':'Configurar'}</button></article>)}</section><section className='crm-card api-contract'><div className='card-head'><div><b>Contrato de integração</b><span>segredos permanecem no servidor</span></div></div><code>GET /api/leads</code><code>POST /api/leads</code><code>PATCH /api/leads/:id</code><code>DELETE /api/leads/:id</code><p>O ZIP inclui <b>src/integrations</b>, validação, .env.example e backend local. Providers externos exigem as chaves correspondentes.</p></section></div>}",
